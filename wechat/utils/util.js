...
function AddHTTPS(imgurl) {
    if (!/^(f|ht)tps?:\/\//i.test(url)) {
        imgurl = "https:" + imgurl; // 替换图像为 https 
    }
    return imgurl;
}
...
function drawTitleExcerpt(context, title, excerpt) {
    ...
    if (getStrLength(title) <= 14) {
        ...
        context.fillText(title.substring(0, 17), 40, 450);
        context.fillText(title.substring(17, 34), 40, 500);
        context.fillText(title.substring(34, 51), 40, 550);
    }
    ...
    for (var i = 0; i <= 50; i += 20) {
        ...
        if (getStrLength(excerpt) > 50) {
            if (i == 40) {
                context.fillText(excerpt.substring(i, i + 20) + "...", 40, 600 + i * 2);
            } else {
                context.fillText(excerpt.substring(i, i + 20), 40, 600 + i * 2);
            }
        } else {
